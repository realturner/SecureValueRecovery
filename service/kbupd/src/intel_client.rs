...
pub fn new_ias_client(host: &str, api_key: Option<&str>, tls_proxy: TlsClientProxySpawner) -> Result<KbupdIasClient, failure::Error> {
    let mut http_connector = HttpConnector::new(1);
    http_connector.enforce_http(false);

    let tls_connector = TlsProxyConnector::new(Arc::new(tls_proxy), http_connector);

    IasClient::new(host, Some(IasApiVersion::ApiVer3), api_key, tls_connector)
}
